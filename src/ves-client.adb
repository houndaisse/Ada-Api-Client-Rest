       ...
       --   Version G1R0 07.12.2020 20H53
         -- Version Amadou avec descripteur JSON.
...
procedure Ves.Client is
   ...
   Server    : constant Swagger.UString := Swagger.To_UString ("http://serveurves:8080"); -- modif amadou
...
begin
   ...
   declare
      ...
      Title   : Swagger.UString;         -- ajout amadou
      Grey    : Swagger.UString;         -- ajout amadou
   begin
      C.Set_Server (Server);
      --if Arg_Count = 2 and Command = "add" then   -- ajout amadou
      Put_Line("avant" & Cr);   
      Title := Swagger.To_UString (Cr);
      Put_Line("apres" & Swagger.To_String (Title));
      
      C.Receive_Event_Using_POST (Title, Grey);
      --end if;
